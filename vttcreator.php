...
for ($rx = $ry = $s = $f = 0; $f < $total; $f++) {
    ...
    if (isset($opts['v'])) {
        ...
        $vtt .= sprintf("%s --> %s\n".$opts['n'].".jpg#xywh=%d,%d,%d,%d", $t1, $t2, $rx++ * $sizes[0], $ry * $sizes[1],  $sizes[0], $sizes[1]);
    }
    $vtt .= "\n\n";
}
...
if (!isset($opts['v'])) {
    imagejpeg($coalesce, "{$params['output']}/".$opts['n'].".jpg", 90);
    for ($s = 0, $f = 0; $f < $total; $f++) {
        unlink($files[$f]);
    }
}

file_put_contents("{$params['output']}/".$opts['n'].".vtt", $vtt);
